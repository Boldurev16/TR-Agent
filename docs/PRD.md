...
- RAG-платформу **AnythingLLM** для семантического поиска и ответов с цитатами;
...
- Автоматический импорт YAML агентов в AnythingLLM (только эталон для ручной настройки).
...
| **Администратор LLM** | Настраивает AnythingLLM, LM Studio/Ollama, эмбеддинги, воркспейс «TR Agent» |

### 3.2. Ключевые user stories

1. **US-1.** Как аналитик, я хочу загрузить PDF в AnythingLLM и задать вопрос по содержимому, чтобы получить ответ с указанием источника.
2. **US-2.** Как оператор, я хочу разбить тысячи PDF на партии по 100 файлов, чтобы загружать их в RAG поэтапно.
3. **US-3.** Как оператор, я хочу извлечь текст PDF в `cache/` с логированием, чтобы не зависеть только от UI AnythingLLM.
...
5. **US-5.** Как администратор, я хочу одной командой проверить Ollama, Docker, AnythingLLM и LM Studio, чтобы быстро локализовать сбои.
...
|----|------------|--------|
| F-ENV-01 | Проверка наличия и доступности Ollama (`11434`) | ✅ |
| F-ENV-02 | Проверка Docker daemon | ✅ |
| F-ENV-03 | Проверка AnythingLLM (`localhost:3001`) | ✅ |
| F-ENV-04 | Проверка LM Studio OpenAI API (`1234/v1`) и списка моделей | ✅ |
...
**TR Agent** — локальная система обработки PDF и Excel-источников через RAG: файлы попадают в файловый конвейер, AnythingLLM индексирует документы, локальная LLM отвечает по контексту.
...
flowchart TB
    ...
    UI --> ALLM
    ALLM -->|"OpenAI API<br/>host.docker.internal"| LM
    ALLM --> VDB
    ALLM -->|"Upload PDF"| BATCHES
    ...
    ENV -.->|"ручной перенос в UI"| ALLM
    MF -.->|"ollama create"| LM
    YAML -.->|"эталон ролей"| ALLM
```

**Потоки данных:**

1. **Оператор → скрипты:** PDF и Excel кладутся в `docs-inbox/`, скрипты готовят батчи, кэш и партии базы знаний.
2. **Оператор → AnythingLLM:** PDF из `docs-batches/` загружаются в воркспейс «TR Agent».
3. **AnythingLLM → LLM:** запросы чата и эмбеддингов уходят на хост через `host.docker.internal`.
4. **Config → UI:** пресеты LLM переносятся в AnythingLLM вручную (не через Git).
...
|------|--------|------------|
| Диагностика | `check_environment.ps1` | Ollama, Docker, AnythingLLM, LM Studio, RAM/GPU, наличие папок |
...
AnythingLLM **не читает PDF с диска напрямую**. Chat-модель видит только то, что попало в промпт после индексации.

**Схема RAG:**

1. PDF загружают в воркспейс → AnythingLLM **извлекает текст**, режет на фрагменты.
2. Строится **векторный индекс** (отдельная **модель эмбеддингов**).
3. При вопросе подмешиваются релевантные фрагменты в промпт.
4. Chat-модель **формулирует ответ по контексту** с citations.
...
sequenceDiagram
    ...
    participant L as LM Studio / Ollama

    U->>A: Вопрос по PDF
    A->>E: Эмбеддинг запроса
    E-->>A: Вектор
    A->>V: Поиск релевантных фрагментов
    V-->>A: Top-K chunks + citations
    A->>L: Prompt + контекст из PDF
    L-->>A: Ответ
    A-->>U: Ответ с источниками
```
...
Файл `config/agents-pdf-research.reference.yaml` описывает три роли. Импорт YAML в AnythingLLM обычно недоступен — это **эталон для ручной настройки** воркспейса «TR Agent».
...
**Варианты внедрения в AnythingLLM (~1.12.x):**

- три воркспейса с разным системным промптом под роль;
- один воркспейс с явными фазами в чате («Сначала разбей задачу… / Проверь индекс… / Ответь с цитатами…»);
- Agent Flows, если используются цепочки.
...
flowchart LR
    ...
    subgraph LMStudio
        ...
        L2["Local Server :1234"]
        L3["Base URL в AnythingLLM:<br/>host.docker.internal:1234/v1"]
    end

    O2 --> ALLM["AnythingLLM Workspace<br/>TR Agent"]
    L3 --> ALLM
```

| Параметр | LM Studio (Docker AnythingLLM) | Ollama |
|----------|-------------------------------|--------|
| Base URL | `http://host.docker.internal:1234/v1` | `http://host.docker.internal:11434` |
| API Key | произвольная строка (`lm-studio`) | не требуется / по версии |
| Model id | из `GET .../v1/models` (напр. `google/gemma-4-31b`) | `my-researcher` (кастом через Modelfile) |
| Переключение модели | загрузить модель в LM Studio **и** обновить id в AnythingLLM | `ollama create` / смена модели в UI |

> **Важно:** из контейнера AnythingLLM `127.0.0.1` — это сам контейнер, **не** Windows-хост. Для LM Studio/Ollama на ПК используйте `host.docker.internal`.
...
### 5.8. Интеграции и порты

| Компонент | Протокол | Адрес (типовой) | Назначение |
|-----------|----------|-----------------|------------|
| AnythingLLM | HTTP | `http://localhost:3001` | UI, RAG, Agent Framework |
| LM Studio | OpenAI-compatible REST | `http://127.0.0.1:1234/v1` (хост) / `host.docker.internal:1234/v1` (из Docker) | Chat-модель, embeddings |
| Ollama | Native API | `http://127.0.0.1:11434` (хост) / `host.docker.internal:11434` (из Docker) | Альтернативный LLM-бэкенд |
| Docker | — | Docker Desktop | Контейнер `mintplexlabs/anythingllm` |
...
2. `scripts/check_environment.ps1` — Ollama, Docker, AnythingLLM `:3001`, LM Studio, RAM, диск, GPU, папки.
...
1. Полный стек Ollama/LM Studio + AnythingLLM Agent Framework.
...
**Внешний стек (не в репозитории):** Docker, AnythingLLM, Ollama и/или LM Studio, embedding-модель для векторного поиска (например, `text-embedding-nomic-embed-text-v1.5`).
...
|-----------|------------|
| Docker Desktop | Контейнер AnythingLLM |
| AnythingLLM (`mintplexlabs/anythingllm`) | RAG, UI, Agent Framework |
...
| **M1** | AnythingLLM + локальная LLM, пилот RAG на 1–3 PDF | 🔲 |
...
| **M5** | Agent Framework по эталону YAML | 🔲 |
...
| Несовпадение model id LM Studio и AnythingLLM | Ошибки или неверная модель | `check_environment.ps1`, пресеты в `config/` |
| Большой объём PDF | Нехватка RAM, медленная индексация | Батчи по 100, `--skip-existing` |
| AnythingLLM UI ≠ YAML агентов | Ручная настройка ролей | Эталон + пошаговые фазы в чате |
